...
fn main() {
    ...
    let read = |rel: &str| -> Vec<u8> {
        ...
        if mgc_import::rnc::is_rnc(&raw) { mgc_import::rnc::decompress(&raw).expect(rel) } else { raw }
    };
    ...
    let (_, name, hmp) = parsed.entries.iter()
        .find(|(_, n, _)| n.starts_with(song_name.as_str())).expect("song not found");
    let song = mgc_import::hmp::parse(hmp).unwrap();
    println!("{name}: {} events, end_tick {}, tick_rate {}", song.events.len(), song.end_tick, song.tick_rate);
    let layered = mgc_import::adlib::has_danger_layer(&song);
    let mix = if layered { mgc_import::adlib::MixSpec::ambient() } else { mgc_import::adlib::MixSpec::full() };
    ...
    println!("rendered {frames} frames ({:.1}s) in {:?}; peak {peak:.3}, rms {rms:.4}, layered {layered}",
        frames as f32 / 44100.0, t0.elapsed());
}
