...
pub fn corner_buckets() -> Vec<Bucket> {
    corner_buckets_for(&CORNER_CLASSES)
}

/// [`corner_buckets`] over an explicit per-texture corner-class table
/// — MC2's `sub_44580` (remc2 Terrain.cpp:1011) runs the SAME 8
/// dihedral insertions over its own table (`unk_D47E0`), so the two
/// games share this generator and differ only in data.
pub fn corner_buckets_for(classes: &[[u8; 4]]) -> Vec<Bucket> {
    let mut buckets = vec![
        Bucket {
            count: 0,
            tex: [0; 12],
    ...
    for (t, corners) in classes.iter().enumerate() {
...
pub fn retile_table() -> Vec<[u8; 2]> {
    retile_table_for(&CORNER_CLASSES)
}

/// [`retile_table`] over an explicit corner-class table (MC2:
/// `building_F2CD0x` generated by `sub_44580` from `unk_D47E0` —
/// same first-candidate/`(1,0)`-default collapse, Terrain.cpp:1093-1117).
pub fn retile_table_for(classes: &[[u8; 4]]) -> Vec<[u8; 2]> {
    corner_buckets_for(classes)
        .iter()
        .map(|b| {
            if b.count != 0 {
                [b.tex[0], b.orient[0]]
