...
**Key: `word_0x96_150` (leader/anchor) is 0 for an AUTHORED map-THING (10,76) spawn** (NewEvent memset, ctor never sets it, `sub_4A310`/`PrepareEvents` never set it — §3, §4). For that path the phase-0 leader branch and `sub_33C70`'s leader-follow branch are **skipped** — the orb is a **free-floating, terrain-clamped** hazard sitting where it was spawned. **2026-07-22 CORRECTION: the leader arm is NOT dead code** — the CHARGED FIREBALL's runtime spawner (`sub_65B50` EF:63012-53, action 29 of the (9,28) projectile, missed by §4's grep because it spawns through `sub_65C20`'s variable `byte_0x43_67/byte_0x44_68` dispatch) writes the projectile's homing lock into the spawned hub's `word_0x96_150` (EF:63029) and overrides maxLife to 30. See §7 OPEN-1 for the full adjudication.
...
## 7. OPEN items
1. **RESOLVED 2026-07-22 — the leader arm is LIVE (the "dead code" call was a grep artifact).** Three-agent adjudication vs remc2 + remc1/remc1hw:
   - The charged fireball (spell 0 tier ≥2 → (9,28)/action 29 `sub_65B50`) spawns the (10,76) at impact via `sub_65C20` and writes the projectile's **homing lock** into the hub's `word_0x96_150` (EF:63029) + maxLife 30 + owner id onto all 25 satellites (local human's satellites swap to sprite 42 via `SetEntityIndex_49C90`, EF:63040-49). A homed hit therefore PINS the firestorm to the struck flyer: phase-0 sizes the ring from the victim's `array_0x52_82.pitch` (wizard 121 → [128,181] shell), `sub_33C70` snaps to it every tick, damage lands every tick, leader death collapses the orb. Terrain hits clear the lock (EF:63196) → free-floating, matching the authored-THING behavior.
   - **Buildings:** remc2 has NO path leadering a struck building (lock scanners exclude the class-3 model-2 castle brain; `sub_65C20` lacks a struck-write), yet retail demonstrably stretches the firestorm over a struck castle's envelope (player-verified; the sizing math — `SetShiftByCastle` pitch = 128·w+640, L6/L7 → bounds [640,3392] in the inverted breathe regime — reproduces the observed "massive fireworks" exactly). The sibling worker `sub_65820` (EF:62991-92, used by nearly every OTHER projectile impact) pins its spawned effect to ANY struck entity, and MC1's independent decompile of the charged worker (`sub_52ED0_53210`, remc1 sub_main.cpp:63194-210) carries a struck-write remc2's `sub_65C20` lacks (gated `class==3 && model<=1` — MC1 wizard balloons; MC1 has no (10,76) orb at all). ADJUDICATION (recorded gameplay outranks the decompile): retail MC2's `sub_65C20` under-transcribes a struck-write; the port leaders the hub with the **struck victim** (proj.rs impact seam's f146 stamp), which equals the lock on a homed hit and covers the castle brain. Retail oddity NOT ported: a deflected homing fireball striking a non-lock entity keeps the ORIGINAL lock as leader; ours pins to the actual struck entity.
   - Remaining sub-questions: which caster path rival wizards use in combat (`sub_3A8B0` is player-generic and would pin symmetrically; `sub_693F0` EF:55858-79 never sets the lock → free-floating), and whether class-5 struck creatures should also leader (big-creature stretch, ring up to [192,576]).
