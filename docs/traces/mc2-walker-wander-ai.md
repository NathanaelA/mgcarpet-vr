...
- **GAME_PAUSED** = bit 1 of OptionsSettingFlag, toggled by `PauseUnpauseGame_18BB0` (PlayerInput.cpp:406-417) and managed around menu-state transitions (GameUI.cpp:662-721). **RESOLVED 2026-07-24 (map-mana-roster.md trace): the fullscreen map DOES set it** — any nonzero map menu state calls `SetPausedMenuOpen_41AF0` (GameUI.cpp:651), so all retail map-dot observations sample a frozen world.
