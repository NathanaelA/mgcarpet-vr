...
The squared 3-D acquisition range is `Entities[self->id_0x1A_26]->dword_0xA0_160x->word_160_0x1c_28` — i.e. the **OWNER's** behavior-row lock range (`v43x = Entities_EA3E4[a1x->id_0x1A_26]`, EF:54781; `v10 = v43x->dword_0xA0_160x->word_160_0x1c_28`, EF:54787). Gate: `sub_583F0_distance_3d(&cand.pos, &self/cand.pos) <= range` (EF:54788). [NOTE — DECOMPILE BUG at EF:54788: the case-0 group passes `&ix->position` for BOTH args (distance-to-self = 0), so the gate is effectively open in that branch; the parallel case-7/9/0x10 branches pass the correct `&cand.pos, &self.pos` (EF:54871, 54897, 54943). Flag as retail-quirk-or-decompile-artifact; a faithful port should use the correct two-point form as in the other branches — OPEN whether the shipped EXE had the same self-self bug.]
...
## 9. OPEN / uncertain
- **EF:54788 self-self distance** in the case-0 group (both args `&ix->position`) — the parallel branches (case 7/9/0x10) use the correct two-point form. Likely a decompile transcription artifact of the case-0 group; a faithful port should use `dist(cand, self)` as the other branches do. Confirm against the shipped EXE if a golden requires it.
