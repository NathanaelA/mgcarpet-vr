...
- **crates/mgc-sim/src/lib.rs::ThrustModel::Enhanced** — An enhancement-class thrust model alongside the faithful MC1 mover: hold-to-fly with automatic deceleration on release, generalizing the original's own hold-to-move strafe to the forward axis. Keeps the authentic level-plane thrust rule (aim pitch never steals mobility). Selected once at the sim boundary; replays record it.
...
- **Gen::mc2_proj_tick** — Non-(9,13) MC2 projectile states fall back to the MC1 projectile handler; the player's spells stay MC1 until the MC2 spell column lands (deliberate cross-column play, the seam's graceful-degradation contract).
...
- **Gen::m22_tick (castle-drain 0xB2/0xB3)** — Reaches through the target player's `CastleEntityIndex_0x3A_58`; no MC2 level spawns a castle today, so the lookup returns None and the machine takes retail's own castle-less arm (LABEL_17 revert). Closes when MC2 castles land.
